...
class Order_Product(Base):
    ...
    order = relationship(
        "Order", backref=backref("order_products", cascade="all, delete-orphan")
    )
    product = relationship(
        "Product", backref=backref("order_products", cascade="all, delete-orphan")
    )
    ...
    def add_products(self, items):
        for product, qty in items:
            self.order_products.append(
                Order_Product(order=self, product=product, quantity=qty)
            )
